...
def leftMouseReleased(event):
    ...
    for cup in currentCups:
        ...
        if (x >= left) and (x<= right) and (y >= top) and (y<= bottom): 
            ...
            if location == "workSpace": # this was previously counter
                cup.onCounter = True
                canvas.cupOnCounter = cup
                canvas.onCounterCount +=1
            elif location == "cauldron":
                canvas.addedToCauldron.append(cup.info)
                print canvas.addedToCauldron
                drawCauldronLiquid(canvas, cup.filledWith)
                redraw(canvas)
                canvas.cupOnCounter = None
    recipe = canvas.recipe
    recipe.checkRecipe(canvas, canvas.addedToCauldron)
...
def find(canvas, anObject): # cup, would also work if moveBottle were enabled
    ...
    workSpaceBounds = ("workSpace", canvas.workSpaceBounds)
    cauldronBounds = ("cauldron",canvas.cauldronBounds)
    areaBounds = [ workSpaceBounds, cauldronBounds]
...
def drawCounter(canvas):
    ...
    workSpaceColor = RGBtoHex(255, 196, 136)
    left = ((canvas.width)/2) + (canvas.width/20)
    top = (canvas.height*3)/4 + canvas.height/50
    right = left + (canvas.width/6)
    bottom = top + (canvas.height/6)
    workSpace = canvas.create_rectangle(left, top, right, bottom, fill = workSpaceColor)
    canvas.workSpaceBounds = (left, (top - canvas.height/12), right, bottom) # add to top to allow cup to be on edge
    workSpaceText = "Prepare ingredients here."
    centerX = (left+right)/2
    centerY = (top+bottom)/2
    scaledFontSize =  canvas.height*canvas.width/(265**2) 
    fontStyle = ("Times New Roman", scaledFontSize, "bold")
    canvas.create_text(centerX, centerY, text = workSpaceText, font = fontStyle)
...
def drawDynamicScene(canvas): # probably a whole different module
    drawCauldron(canvas)
    if canvas.potionColor != None:
        drawCauldronLiquid(canvas, canvas.potionColor)
    drawBottles(canvas)
...
def run():
    ...
    canvas.recipeName = "Good Luck Potion" # default starting recipe
    init(canvas)
    # set up events
