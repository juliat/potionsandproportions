...
def keyPressed(event):
    ...
    if (event.char == "n"):  #(canvas.recipe.complete == True) and 
        ...
        init(canvas, "yes")
    if event.char == "u":
        canvas.myCauldron.removeFromCauldron()
        canvas.myCauldron.drawCauldronLiquid()
        displayProgress(canvas)
        canvas.undoCount +=1 # may limit number of undos
        redraw(canvas)
...
##    alreadyPlayed = canvas.alreadyPlayed
##    poss = [ ]
##    for recipe in allRecipes:
##        if recipe not in canvas.alreadyPlayed:
##            poss.append(recipe)
...
##    alreadyPlayed.append(randomRecipe)
...
def displayProgress(canvas):
    canvas.update
    progressMessage = canvas.progressMessage
    left = (canvas.width/2)
    top = canvas.height/3
