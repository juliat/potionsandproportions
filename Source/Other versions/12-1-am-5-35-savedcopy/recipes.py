...
class Recipe:
    ...
    def checkRecipe(self, inCauldron):
        ...
        if (inCauldron.keys() == [ ]):
            ...
            for ingredient in correctAmounts.keys():
                if inCauldron.get(ingredient) == correctAmounts.get(ingredient):
            ...
            if allCorrect == True:
                ...
                canvas.difficulty +=1
                self.complete = True
            for ingredient in inCauldron.keys():
...
recipes["Slug Repellent"]["ingredients amounts"] ={'pamplemousse':1.0, 'fluxweed juice':(1/3.0), 'armadillo bile':(1/4.0), 'lily venom':(1/3.0)}
...
recipes["Ageing Potion"]["ingredients amounts"] = {'lily venom':1.0, 'armadillo bile':1.0, 'leech juice':(1/3.0), 'pamplemousse':(1/4.0)}
